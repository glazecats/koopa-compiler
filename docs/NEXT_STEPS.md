...
- Current progress snapshot for that ordered line:
  - strict whole-repo runtime-RE audit:
    **in progress / ~98%**
    ...
    - 2026-05-11 tail-probe follow-up: three new multi-structure probe suites were generated around the user-specified "if (cond) x = new_value; return use(x);" tail shape, but all of them stayed green on the live tree. The probe sets were `/tmp/runtime_re_tail_probe_suite` (`16/16 PASS`), `/tmp/runtime_re_tail_probe_suite2` (`32/32 PASS`), and `/tmp/runtime_re_tail_probe_suite3` (`36/36 PASS`), covering direct if / if-else / nested-if / short-circuit / loop / call-chain / sequential-if variations with local, global, array, and call-based tails. Current authority after this follow-up is therefore that this whole nearby tail-return family is *not* currently exposing the remaining hidden runtime-RE line, so the search should rotate to a different adjacent family again.
...
- 2026-05-11 clarification follow-up: the `m(f(1), h(2), f(3))` witness turned out to be a **求值顺序歧义伪 witness** rather than a stable compiler bug signal. The repository's actual lowering contract is still left-to-right for call arguments, so the host-oracle comparison that disagreed there was not a clean bug oracle. I replaced that with a deterministic call/side-effect probe family where each full-expression keeps a single side-effectful call, and the new `/tmp/runtime_re_deterministic_funcexpr_safe` batch is now fully green (`48/48 PASS`), so the active search should continue from the broader later-call / stack-arg / address-carrier family rather than the ambiguous nested-argument shape itself.
    - 2026-05-11 compiler-driver isolation follow-up: a brief attempt to make `COMPILER_ENABLE_AGGRESSIVE_OPTIMIZATIONS` default to conservative immediately broadened the behavior change much more than intended. That single toggle does **not** only gate the final RISC-V text peepholes; it also switches `.sbss/.sdata` vs `.bss/.data`, the `value_ssa_build_default_from_lower_ir(...)` vs `value_ssa_build_from_lower_ir(...)` entry, `value_ssa_optimize_perf_hotspots(...)`, and whether the text exporter injects the caller-save save/restore wrapper around calls. Because that broad toggle produced a judge-side `AE`, the live tree now keeps the historical aggressive-default behavior while exposing narrower env-isolation toggles in `compiler_driver.c`:
      ...
      Local recheck after this split: `make` PASS and `autotest -riscv -s lv8` PASS (`12/12`). Current next step is to use these narrower toggles to identify which one actually correlates with the judge-side `AE`, instead of changing all driver behaviors at once.
    - 2026-05-11 caller-save follow-up: enabling `COMPILER_USE_CALLER_SAVE_TEXT` is now confirmed to be a separate bug line from the earlier `AE`. The first restore placement bug was fixed by moving the restore point to after the explicit `jalr` in caller-save mode, and the external/internal call printing was split so external helpers still keep the symbolic `call foo` form. However, the caller-save path still currently produces runtime corruption on caller-heavy `lv9` shapes (`05_global_arr_init`, `08_arr_access`, `09_const_arr_read`, `12_more_arr_params`, `13_complex_arr_params` show `-11` in the caller-save mode sweep), so the remaining open work on this branch is now narrowed to live-value / frame-layout correctness rather than to assembler syntax. Current suspicion is that the caller-save save area or a caller-clobbered live value is still being handled too aggressively around some call sites.
    - 2026-05-11 simple-backend shrink follow-up: the diagnostic simple-backend branch is now materially simpler than the first draft. `machine_ir` has an explicit dedicated all-spill entrypoint (`machine_ir_build_all_spill_flat_program_only_report(...)`), the simple backend now uses that all-spill report as its only path instead of the old shared allocate+rewrite report path, and the conservative env/profile isolation for this branch now lives in `compiler_driver.c` instead of the simple-backend helper itself. The default submission path now goes through this simple backend unless `COMPILER_DISABLE_SIMPLE_BACKEND=1` is set. Focused rechecks after this shrink stayed green on the relevant public surfaces: default `autotest -riscv -s lv8` PASS (`12/12`), `COMPILER_SIMPLE_BACKEND=1 autotest -riscv -s lv8` PASS (`12/12`), and `COMPILER_SIMPLE_BACKEND=1 autotest -riscv -s lv9` PASS (`22/22`). `make test-machine-ir` still stops on the already-known `spill.3/spill.4` dump-shape expectation noise, so that target remains a non-clean checkpoint surface for this runtime-RE line rather than evidence of a new regression from the simple-backend shrink.
    - 2026-05-11 simple-backend AE debug follow-up: the direct simple-text emitter path is now gated behind `COMPILER_USE_DIRECT_SIMPLE_TEXT=1` instead of being the default. The default simple-backend route now prefers the bytes/text export chain, which is a narrower diagnostic surface for the newly reported `23_func_while3: AE` while still keeping the direct path available for local comparison. This is a deliberate compatibility-first shrink, not a proof that the original direct emitter was definitively wrong.
