...
- Current progress snapshot for that ordered line:
  - strict whole-repo runtime-RE audit:
    **in progress / ~95%**
    - current active reread boundary is no longer only the compiler-side
      ...
      the latest concrete minimized witness has reopened
      `src/ir/ir_lower_stmt.inc` loop-exit fact handling as the current
      front-most kept closure inside the same generated-program runtime-RE
      audit
    ...
    - broader generated-program runtime repro hunting is now **~99%**:
      ...
      round
      while keeping the rotated hidden-like batch green
