...
- `value_ssa_build_canonicalized_from_lower_ir(...)` is the preferred one-shot bridge when a caller wants the current result-layer cleanup/canonicalization pipeline applied immediately after conversion.
- The preferred one-shot bridge now also acts as a stabilized-output boundary: it runs an extra canonicalization convergence step internally, so representative bridge-authority shapes should already be fixed-point if a caller chooses to canonicalize them once more.
...
- The same preservation policy also applies to the one-shot lower-IR bridge: conversion followed by canonicalization must preserve dead-result `call` instructions rather than treating them as removable cleanup noise.
