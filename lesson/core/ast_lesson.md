...
`AST` 是 parser 和 semantic 之间最关键的共享结构层，它把“语法已经解析出来的东西”稳定成可被下游消费的数据模型。

## 常见误解

- 误解一：AST 只是 parser 的内部中间产物。
  - 实际上 semantic、测试、后续 lowering 都会依赖这里定义出来的结构和字段语义。
- 误解二：AST 只是节点枚举表。
  - 当前这层同样有所有权、生命周期、失败路径清理这些真实工程边界。
...
- parser 到底产出了什么结构
- 这些结构最后如何在 `ast.h` 里稳定表达出来

如果你不先看 parser，AST 里的很多节点种类会像是静态枚举表，而不是“真实解析产物”。
