...
`ir` 是 semantic 之后第一个稳定、可验证、可被后续 pass 和 lower_ir 共同消费的程序表示层。

## 常见误解

- 误解一：IR 只是把 AST 换个写法保存。
  - 实际上从这层开始，程序已经被重组为 function / block / instruction / terminator 的执行导向表示。
- 误解二：有了 IR 就已经接近 machine backend 了。
  - 当前 canonical IR 仍然离 lower IR、SSA、machine backend 还有明确的几层边界。
...
那读 IR 时很容易把“语义决定”和“IR 表示决定”混在一起。

## 读完后接哪篇

最自然往下接：

1. `lesson/core/ir_pass_lesson.md`
2. `lesson/core/lower_ir_lesson.md`

如果你更关心：

- canonical IR 怎么继续被清理
  - 先接 `ir_pass`
- canonical IR 怎么继续往后端入口落
  - 先接 `lower_ir`
