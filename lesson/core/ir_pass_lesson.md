...
`ir_pass` 是 canonical IR 建好之后的第一条“结构化整理与小型优化管线”，它负责把已经合法的 IR 进一步清成更稳定、更适合后续 lowering 的形状。

## 常见误解

- 误解一：IR pass 只是可有可无的小优化。
  - 当前这条线已经承担 fold、copy、const、cfg、dce 和 default pipeline 这样的稳定整理责任。
- 误解二：IR lowering 自己就会顺便把这些都做掉。
  - 现在 `ir` 和 `ir_pass` 已经明确分层：先建 IR，再按需跑 pass。
...
- canonical IR 的基本结构
- block / instruction / terminator 是怎么组织的

那 `ir_pass` 里很多 fold / copy / cfg / dce 的动作会缺少上下文。
...
- 清理过的 canonical IR 怎么继续降到 lower IR
- 这些 pass 行为又是怎么被测试锁住的
