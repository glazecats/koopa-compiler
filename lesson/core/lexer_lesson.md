...
`lexer` 是整条前端主线里最早把源码字符流变成结构化输入的层，它把“原始文本”第一次收束成 token stream。

## 常见误解

- 误解一：lexer 只是机械切字符串，没什么设计边界。
  - 实际上 token 分类、位置信息、最大吞噬、注释处理、错误 token 都会直接影响后面的 parser。
- 误解二：只要 token 大概切对就行。
  - 对 parser 和测试来说，token 类型、长度、位置、边界行为都属于真实 contract。
...
这篇基本可以直接读，不强依赖别的 lesson。

如果你想读得更顺，可以先看：

1. `lesson/README.md`

知道自己现在是在整条 `core -> ssa -> machine` 大主线的最前端。
