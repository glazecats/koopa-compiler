...
`lower_ir` 是 canonical IR 和后续 SSA / backend 之间的第一道“表示收窄层”，它最核心的动作是把 value 和 slot 明确拆开。

## 常见误解

- 误解一：lower IR 只是 canonical IR 多几个 opcode。
  - 更准确地说，它是新的 downstream IR，而不是原地给 canonical IR 打补丁。
- 误解二：lower IR 已经是 machine IR。
  - 它还远没有进入 machine register / selected op 世界，只是先把 value-storage 边界讲清楚。

## 导学

这份讲义建议按“设计边界 -> 数据模型 -> lowering -> dump/verifier -> 测试”的顺序阅读：

1. 先看 `docs/ir/LOWER_IR_DESIGN.md`，理解为什么 lower IR 是一层新的 downstream IR。
...
2. `lesson/core/ir_pass_lesson.md`

如果你还没先搞清楚：

- canonical IR 的基本形状
- canonical IR pass 已经清理到什么程度

那读 lower IR 时会不知道“哪些工作还是 canonical IR 负责，哪些已经换到 lower IR 负责”。
...
2. `lesson/ssa/memory_ssa_lesson.md`
3. `lesson/machine/README.md`

最常见的两条后续路线是：

- 想继续走 SSA 主线
  - 先接 `value_ssa_lesson.md`
- 想先建立整体后端路线感
  - 先接 `lesson/machine/README.md`
