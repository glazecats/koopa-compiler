...
`parser` 是把 token stream 组织成 AST 结构的那一层，它回答的核心问题不是“这段代码合不合法语义”，而是“这串 token 按语法应该长成什么树”。

## 常见误解

- 误解一：parser 已经在决定大部分语义规则。
  - 更准确地说，parser 主要决定语法形状；很多名字、作用域、调用约束要到 semantic 才真正定下来。
- 误解二：parser 只是把 token 顺着扫一遍。
  - 实际上这里已经有递归下降层次、优先级处理、函数与声明回退等结构化决策。
...
因为 parser 的真实输入不是源码字符串，而是 lexer 已经切好的 token stream。

如果你还没先建立：

- token 大概有哪些种类
- lexer 会把哪些源码片段切成什么 token

那 parser 这里很多 `peek/match/consume` 决策会比较抽象。

## 读完后接哪篇

最自然往下接：

1. `lesson/core/ast_lesson.md`
2. `lesson/core/semantic_lesson.md`

最常见的读法是：

- 先接 `ast_lesson`
  - 看 parser 产物最后如何稳定落到 AST
- 再接 `semantic_lesson`
  - 看这些 AST 怎么继续被语义层消费
