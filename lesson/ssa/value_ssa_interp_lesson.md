...
`value_ssa_interp` 是 SSA 这一侧的执行语义层，它不是优化 pass，而是 verifier-legal SSA program 的窄解释器与测试 oracle。

## 常见误解

- 误解一：解释器只是测试工具，不算正式模块。
  - 当前它已经是独立 sibling module，并且承担 oracle / execution semantics 基础职责。
- 误解二：它和 machine runtime 是同一类东西。
  - 这里执行的是 SSA 表示，不是 machine byte / machine state / runtime process world。
...
- SSA program / block / instruction / phi 是什么

那解释器这里的执行状态、block 进入、value 读取都会比较抽象。
...
- 想回到 SSA 主线继续往 allocator 走
  - 先接 `value_ssa_alloc`
- 想理解 oracle / interpreter 在测试里的用法
  - 先接 `tests_lesson`
