...
下面先看一个 optimistic coloring 的例子。这个例子不是从某段 C 源码自然编译出来的，
而是 allocator 层的 regression witness。测试直接构造一个很小的
interference / coalescing 配置，用来锁定“高风险值在 select 阶段如何通过
eviction/recolor 保住颜色”的策略。这样做的好处是变量少、压力关系清楚；
...
它可能直接落入 spill 或破坏已有颜色安排。真实 dump 的关键片段如下：
...
对应 regression 会断言：\texttt{ssa.0} 走了 eviction，blocker 是 \texttt{ssa.2}，
且被 recolor 到 \texttt{color=1}。所以它主要说明 optimistic-colored：\texttt{spill-remove}
只是 planner 的风险预测，select 还能用局部 recolor 救回寄存器；但选择
\texttt{ssa.2} 而不是 \texttt{ssa.1}，并不是单纯按 id 兜底：两种 recolor 都能形成 family 共色，
\texttt{ssa.2}/\texttt{ssa.5} 的共色 weight=3，高于 \texttt{ssa.1}/\texttt{ssa.3} 的 weight=1。

更加直接体现 family-aware 的地方，可以看下面这个 witness。这个 witness 在测试中名为
\path|manual_main_family|，它把颜色数压到 2，并且让目标值
\texttt{ssa.0} 同时面对两个可牺牲的 blocker：
...
manual family facts:
  family A: {ssa.1, ssa.3}, root = ssa.1, size = 2
  family B: {ssa.2}, root = ssa.2, size = 1

selected interference:
  ssa.0 conflicts with ssa.1 and ssa.2
  ssa.1 conflicts with ssa.2

removal plan:
  ssa.0: spill-remove, priority = 30, spill_cost = 3
  ssa.1: simplify-remove, priority = 10, spill_cost = 2
  ssa.2: simplify-remove, priority = 10, spill_cost = 2
  ssa.3: simplify-remove, priority = 5,  spill_cost = 2
\end{lstlisting}

这里关键不是 \texttt{ssa.0} 最后能不能 optimistic-colored，而是为什么 selector
愿意为了它挪开 blocker，以及当 \texttt{ssa.1} 和 \texttt{ssa.2} 都可以作为 blocker
被牺牲时 allocator 怎样选择。先看第一层原因：\texttt{ssa.0} 的 \texttt{spill\_cost=3}，
而两个 blocker 的 \texttt{spill\_cost} 都是 2，所以牺牲一个较低 cost 的 blocker，
可以保住一个更贵的 spill 候选；这就是 eviction/recolor 值得发生的原因。
在纯标量 tie-break 里，两个 blocker 的局部分数很接近，甚至很容易退化成按 value id
或偶然顺序选择；family-aware 版本会多看一步：\texttt{ssa.1} 属于
\texttt{\{ssa.1, ssa.3\}} 这个 size=2 的 coalesced family，而 \texttt{ssa.2}
只是一个 singleton family。因此主 select 的 generic eviction 会优先牺牲更小的
family，也就是 \texttt{ssa.2}：

\begin{lstlisting}[language={}]
select-trace colors=2 values=4
  1: ssa.2 remove=simplify-remove
     spill_cost=2(uses=1,blocks=1,affinity=0)
     outcome=spill-confirmed
  3: ssa.0 remove=spill-remove
     spill_cost=3(uses=2,blocks=1,affinity=0)
     outcome=optimistic-colored color=0

allocation checks:
  ssa.0 -> color0
  ssa.1 -> colored, not spilled
  ssa.2 -> spill-confirmed
\end{lstlisting}

这个例子说明 family-aware 不是把 family 合成大节点，而是在牺牲 blocker 前，先看是否值得
用低代价 blocker 保住高代价 value，再参考 recolor 可行性、family 当前颜色支持和 pressure。
前一个例子偏 optimistic-colored，这个例子才体现 family-aware 的 blocker 取舍。

和传统“放进栈里、逆序染色”的朴素版本相比，这里的差别不在于栈式 select 消失了，
而在于 select 时多了可解释的偏好、恢复动作和 family 取舍。朴素实现通常只回答
“这个点有没有颜色”；这个 allocator 还能回答“为什么它想要这个颜色、是谁挡住了它、
有没有把 blocker 挪开、牺牲哪个 blocker 对 family 破坏更小、最后是不是真的 spill”。
在热点代码里，这类策略可以减少无意义 move，避免把本来属于同一 value family 的值随意拆散；
更重要的是，它让 allocator 的决策能被 dump 和 regression test 直接锁住，而不是依赖主观判断。
