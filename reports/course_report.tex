...
blocker 是 \texttt{ssa.2}，并且 blocker 被 recolor 到 \texttt{color=1}。所以这个例子
更适合放在 optimistic-colored 的位置：它说明 \texttt{spill-remove} 只是 planner 的风险
预测，最终 select 仍然可以通过局部 recolor 把值救回寄存器。

真正体现 family-aware 的地方，可以看下面这个 witness。这个 witness 在测试中名为
