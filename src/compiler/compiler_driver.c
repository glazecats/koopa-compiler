...
static int compiler_riscv_preview_line_is_mv_reg(
    const char *line,
    char *rd,
    size_t rd_size,
    char *rs,
    size_t rs_size) {
    if (!line || !rd || rd_size == 0u || !rs || rs_size == 0u) {
        return 0;
    }
    rd[0] = '\0';
    rs[0] = '\0';
    return sscanf(line, "  mv %31[^,], %31s", rd, rs) == 2;
}
...
static int compiler_riscv_preview_lines_load_same_constant_immediate(
    char **lines,
    size_t line_count,
    size_t start_index,
    char *rd,
    ...
    if (!lines || !rd || rd_size == 0u || !out_imm_value || !out_pattern_len || start_index >= line_count) {
        return 0;
    }

    if (compiler_riscv_preview_line_loads_immediate(lines[start_index], rd, rd_size, out_imm_value)) {
        *out_pattern_len = 1u;
        return 1;
    }
    if (start_index + 1u >= line_count ||
        !compiler_riscv_preview_line_is_lui_imm(lines[start_index], lui_rd, sizeof(lui_rd), lui_imm, sizeof(lui_imm)) ||
        !compiler_riscv_preview_line_is_addi_reg_imm(
            lines[start_index + 1u], addi_rd, sizeof(addi_rd), addi_rs1, sizeof(addi_rs1), addi_imm, sizeof(addi_imm)) ||
        strcmp(lui_rd, addi_rd) != 0 ||
        strcmp(lui_rd, addi_rs1) != 0 ||
        !compiler_parse_signed_immediate_text(lui_imm, &upper) ||
        !compiler_parse_signed_immediate_text(addi_imm, &lower)) {
        return 0;
    }
    ...
    snprintf(rd, rd_size, "%s", lui_rd);
    *out_imm_value = (int)value;
    *out_pattern_len = 2u;
    return 1;
}
...
static int compiler_riscv_preview_line_is_conditional_branch_using_reg(
    const char *line,
    const char *reg_name) {
    char tokens[8][32];
    size_t token_count = 0u;
    ...
    if (strcmp(tokens[0], "beq") != 0 &&
        ...
        strcmp(tokens[0], "blt") != 0 &&
        strcmp(tokens[0], "bge") != 0 &&
        strcmp(tokens[0], "bltu") != 0 &&
        strcmp(tokens[0], "bgeu") != 0) {
        return 0;
    }
...
static int compiler_optimize_riscv_preview_mod998_divmods(char **io_text) {
    ...
    while (index < line_count) {
        ...
        if (compiler_riscv_preview_lines_load_same_constant_immediate(
...
int compiler_test_optimize_riscv_preview_mod998_divmods(char **io_text) {
    return compiler_optimize_riscv_preview_mod998_divmods(io_text);
}
...
static int compiler_optimize_riscv_preview_branch_bound_stack_reload_to_mv(char **io_text) {
    char *text = NULL;
    char *copy = NULL;
    char **lines = NULL;
    size_t line_count = 0u;
    ...
    while (index < line_count) {
        ...
        size_t scan_index;
        int replaced = 0;
        char previous_store_reg[32];
        int previous_store_offset = 0;
        int previous_materialized_stack_offset = 0;
        ...
        if (compiler_riscv_preview_line_is_sw_sp_reg(lines[index], store_reg, sizeof(store_reg), &store_offset) &&
            !(index > 0u &&
                compiler_riscv_preview_line_is_sw_sp_reg(
                    lines[index - 1u],
                    previous_store_reg,
                    sizeof(previous_store_reg),
                    &previous_store_offset) &&
                previous_store_offset == store_offset) &&
            !(index > 1u &&
                compiler_riscv_preview_two_line_is_sw_materialized_stack_slot(
                    lines,
                    index - 2u,
                    &previous_materialized_stack_offset) &&
                previous_materialized_stack_offset == store_offset) &&
            compiler_riscv_preview_is_temp_reg_name(store_reg)) {
            for (scan_index = index + 1u; scan_index + 1u < line_count; ++scan_index) {
                char load_reg[32];
                char overwrite_reg[32];
                int load_offset = 0;
                int materialized_stack_offset = 0;
                ...
                if (!compiler_riscv_preview_line_is_lw_sp_reg(
                    ...
                    load_offset != store_offset ||
                    !compiler_riscv_preview_is_temp_reg_name(load_reg) ||
                    !compiler_riscv_preview_line_is_conditional_branch_using_reg(lines[scan_index + 1u], load_reg)) {
                    continue;
                }
...
int compiler_test_optimize_riscv_preview_branch_bound_stack_reload_to_mv(char **io_text) {
    return compiler_optimize_riscv_preview_branch_bound_stack_reload_to_mv(io_text);
}

static int compiler_optimize_riscv_preview_forward_store_copy_source(char **io_text) {
    char *text = NULL;
    char *copy = NULL;
    char **lines = NULL;
    size_t line_count = 0u;
    ...
    while (index < line_count) {
        ...
        if (compiler_riscv_preview_line_is_mv_reg(lines[index], copy_rd, sizeof(copy_rd), copy_rs, sizeof(copy_rs))) {
            for (scan_index = index + 1u; scan_index < line_count; ++scan_index) {
                char store_reg[32];
                char store_base[32];
                int store_offset = 0;
                ...
                if (compiler_riscv_preview_line_is_label(lines[scan_index]) ||
                    compiler_riscv_preview_line_is_control_transfer_barrier(lines[scan_index]) ||
                    compiler_riscv_preview_line_defines_reg(lines[scan_index], copy_rd) ||
                    compiler_riscv_preview_line_defines_reg(lines[scan_index], copy_rs)) {
                    break;
                }
                if (!compiler_riscv_preview_line_is_sw_reg_offset(
                        lines[scan_index], store_reg, sizeof(store_reg), &store_offset, store_base, sizeof(store_base)) ||
                    strcmp(store_reg, copy_rd) != 0) {
                    if (compiler_riscv_preview_line_uses_reg(lines[scan_index], copy_rd)) {
                        break;
                    }
                    continue;
                }
...

                {
                    ...
                    for (emit_index = index + 1u; emit_index < scan_index; ++emit_index) {
                        if (!compiler_builder_appendf(&builder, "%s\n", lines[emit_index])) {
                            goto cleanup;
                        }
                    }
                    if (!compiler_builder_appendf(&builder, "  sw %s, %d(%s)\n", copy_rs, store_offset, store_base)) {
                        goto cleanup;
                    }
                }
                replaced = 1;
