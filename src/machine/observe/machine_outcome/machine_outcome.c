...
int machine_outcome_file_get_source_elf_artifact_summary(
    const MachineOutcomeFile *outcome_file,
    MachineElfArtifactSummary *out_summary) {
    if (!outcome_file || !out_summary) {
        return 0;
    }
    return machine_event_file_get_source_elf_artifact_summary(&outcome_file->event_file, out_summary);
}
...
int machine_outcome_dump_file(
    ...
    if (!machine_outcome_append_format(
            &builder,
            "machine_outcome profile=%s elf_origin=%s elf_semantics=%s event=%s origin-status=%s origin-pc=0x%zx origin-sp=0x%zx origin-segment=%zu mapped_bytes=%zu\n",
            machine_elf_target_profile_name(header_summary.target_profile),
            machine_elf_target_profile_name(
                outcome_file->event_file.trace_file.delta_file.observe_file.apply_file.commit_file.writeback_file
...
int machine_outcome_report_get_source_elf_artifact_summary_artifact(
    ...
    *out_summary = &report->file.event_file.trace_file.delta_file.observe_file.apply_file.commit_file
                        .writeback_file.mutation_file.state_file.transition_file.interp_file
...
int machine_outcome_dump_report(
    ...
    if (!machine_outcome_append_format(
            ...
            machine_elf_target_profile_name(
                report->file.event_file.trace_file.delta_file.observe_file.apply_file.commit_file.writeback_file
                    .mutation_file.state_file.transition_file.interp_file.payload_decode_file.decode_file
                    .step_file.launch_file.runtime_file.load_file.exec_file.image_file
                    .source_elf_artifact_summary.origin_profile),
            machine_elf_relocation_semantics_name(
